---
title: "Extending NIMBLE: Introduction to nimbleEcology"
...
The `nimbleEcology` package (Goldstein et al.) provides *marginal* distributions for:
...
Consider the occupancy example:
...
\]
\[
P(y[i,1\colon J] ~|~ z[i]) = \prod_{j=1}^J (z[i] p[i,j])^{y[i,j]} + (1-z[i]p[i,j])^{1-y[i,j]}
\]

`y[i, 1:J]` is a vector random variable.

MCMC must sample all `z[i]` values (dimensions of the model space).

# Marginal occupancy distribution
...
\[
\begin{array}{rl}
P(y[i,1\colon J]) = & \psi \prod_{j=1}^J \mbox{dbern}(y[i,j] ~|~ p[i,j]) 
                  + & (1-\psi) I(\mbox{all } y[i, j] \mbox{ = 0})
\end{array}
\]

This sums over possible values of `z[i]`, so the `z[i]` are "marginalized" out of the model. They do not need to be sampled by MCMC.

# Marginal occupancy distribution in code

We want to be able to re-write the occupancy model something like this:
...
Really there will be different versions of "`dOcc`" with different names.

# Marginal distributions in `nimbleEcology`

- CJS: Marginalize over alive/dead state in Cormack-Jolly-Seber (capture-recapture) models
- Occupancy: Marginalize over site occupancy state
- Dynamic occupancy: Marginalize over site occupancy state through time
- N-mixture: Marginalize over site abundance
- Hidden Markov Model (HMM): Marginalize over sequence of latent states

    - Used for multi-state and multi-event capture-recapture models.
...
# Why marginalize? Algorithm efficiency
...
- Marginalizing can change both computation time and mixing (ESS).
- Sometimes fancy samplers are too slow (computationally costly) to be worthwhile.

#### Other details on how we will compare MCMC efficiency.
...
- From [*Bayesian Population Analysis Using WinBUGS*](https://www.vogelwarte.ch/en/research/population-biology/book-bpa/) (K&#233;ry and Schaub 2011), aka BPA.
